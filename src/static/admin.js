...
document.addEventListener('DOMContentLoaded', function() {
    ...
    openCameraBtn.onclick = function() {
        navigator.mediaDevices.getUserMedia({ video: true })
            ...
            .catch(() => { adminResult.textContent = 'Camera access denied.'; });
    };

    registerForm.onsubmit = function(e) {
        e.preventDefault();
        const empId = registerForm.employee_id.value;
        ...
        if (adminVideo.style.display === 'block') {
            ...
            adminCanvas.toBlob(function(blob) {
                submitRegistration(empId, blob);
            }, 'image/jpeg');
        } else if (imageFile) {
            submitRegistration(empId, imageFile);
        } else {
            adminResult.textContent = 'Please provide an image.';
        }
    };

    function submitRegistration(empId, imageBlob) {
        ...
        .then(data => {
            if (data.status === 'success') {
                adminResult.textContent = 'Employee registered! ID: ' + data.employee_id;
            } else {
                adminResult.textContent = data.message;
            }
        })
        .catch(() => { adminResult.textContent = 'Error registering employee.'; });
    }
});
